...
右側分成**工作區首頁、目前工作、任務紀錄**三個獨立分頁；導覽列捲動時會留在上方，背景覆蓋完整內容寬度；開啟任務或 SuperSplat 時會回到頂部，讓操作工具列完整顯示。
...
- **依用途找任務**：類別固定顯示「影片抽幀、照片重建（COLMAP）、模型訓練（3DGS）、網格模型（Mesh）、雲端傳輸、深度與法線、影像去背、大場景分塊」，搭配簡短用途說明。
